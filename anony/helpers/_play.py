...
def checkUB(play):
    async def wrapper(_, m: types.Message):
        ...
        # Fetch panel settings once for all checks below
        panel = await db.get_panel()

        # Maintenance mode — only sudoers can use the bot
        if panel.get("maintenance") and m.from_user.id not in app.sudoers:
            return await m.reply_text(
                "🔧 <b>Maintenance Mode</b>\n\nThe bot is currently under maintenance. Please try again later."
            )

        # Force subscribe — user must be a member of the configured channel
        if panel.get("fsub") and panel.get("fsub_id"):
            try:
        ...
        queue_limit = panel.get("queue_limit", config.QUEUE_LIMIT)
        if len(queue.get_queue(chat_id)) >= queue_limit:
            return await m.reply_text(m.lang["play_queue_full"].format(queue_limit))

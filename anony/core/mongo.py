...
class MongoDB:
    ...
    # PANEL SETTINGS
    async def load_panel(self) -> None:
        doc = await self.panel_db.find_one({"_id": "settings"}) or {}
        self.panel = {
            "maintenance": doc.get("maintenance", False),
            "fsub": doc.get("fsub", False),
            "fsub_id": doc.get("fsub_id", None),
            "fsub_username": doc.get("fsub_username", None),
            "duration_limit": doc.get("duration_limit", config.DURATION_LIMIT // 60),
            "queue_limit": doc.get("queue_limit", config.QUEUE_LIMIT),
            "thumb_gen": doc.get("thumb_gen", config.THUMB_GEN),
            "video_play": doc.get("video_play", config.VIDEO_PLAY),
        }

    async def get_panel(self) -> dict:
        return self.panel

    async def set_panel(self, key: str, value) -> None:
        self.panel[key] = value
        await self.panel_db.update_one(
            {"_id": "settings"},
            {"$set": {key: value}},
            upsert=True,
        )
